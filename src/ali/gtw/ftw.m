function ali = ftw(Xs, bas, ali0, aliT, parFtw)
% Gauss-Netwon algorithm for GTW.
% 
% Input
...
lam = ps(parFtw, 'lam', 0);
th = ps(parFtw, 'th', 0);
isNor = psY(parFtw, 'nor', 'n');
nItMa = ps(parFtw, 'nItMa', 100);
qp = ps(parFtw, 'qp', 'mosek');
isDebg = psY(parFtw, 'debg', 'n');
fig = ps(parFtw, 'fig', 11);
...
for nIt = 1 : nItMa
    ...
    if nIt == 1
        ...
        % v
        vs = cellVec(seqInp(Xs, ali0.P, parFtw));

        % G
        XDPs = seqInp(XDs, ali0.P, parFtw);
        for i = 1 : m
        ...
        H = m * GGDiag - GHorz' * GHorz + lam * PDDiag;
    ...
    % debug
    ali.P = P;
    objs(nIt) = gtwObj(Xs, ali, [], parFtw);
    debg(isDebg, Ax1, Ax2, nIt, objs, XXs, bas, aliT, ali, parFtw);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function debg(isDebg, Ax1, Ax2, nIt, objs, XXs, bas, aliT, ali, parFtw)
% Show current status in the optimization of FTW.
...
parChan = st('nms', {'x', 'y', 'z'}, 'gapWid', 1, 'all', 'y', 'inp', parFtw.inp);
...
% bases & weight
for i = 1 : m
    shP(Ps{i}, 'ax', Ax2{i}, 'n', ns(i), 'mkSiz', 0);
